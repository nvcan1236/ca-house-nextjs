import { z } from "zod"

const configSchema = z.object({
  NEXT_PUBLIC_GOOGLE_API_KEY: z.string(),
  NEXT_PUBLIC_GOOGLE_AUTH_DOMAIN: z.string(),
  NEXT_PUBLIC_GOOGLE_PROJECT_ID: z.string(),
  NEXT_PUBLIC_GOOGLE_STORAGE_BUCKET: z.string(),
  NEXT_PUBLIC_GOOGLE_MESSAGING_SENDER_ID: z.string(),
  NEXT_PUBLIC_GOOGLE_APP_ID: z.string(),
  NEXT_PUBLIC_GOOGLE_MEASUREMENT_ID: z.string(),

  NEXT_PUBLIC_GOOGLE_AUTH_URI: z.string(),
  NEXT_PUBLIC_GOOGLE_TOKEN_URI: z.string(),
  NEXT_PUBLIC_GOOGLE_REDIRECT_URIS: z.string(),
  NEXT_PUBLIC_GOOGLE_CLIENT_ID: z.string(),

  NEXT_PUBLIC_GEO_API_KEY: z.string(),
  NEXT_PUBLIC_MAP_TOKEN: z.string(),
  NEXT_PUBLIC_BE_URL: z.string(),
})

const configProject = configSchema.safeParse({
  NEXT_PUBLIC_GOOGLE_API_KEY: process.env.NEXT_PUBLIC_GOOGLE_API_KEY,
  NEXT_PUBLIC_GOOGLE_AUTH_DOMAIN: process.env.NEXT_PUBLIC_GOOGLE_AUTH_DOMAIN,
  NEXT_PUBLIC_GOOGLE_PROJECT_ID: process.env.NEXT_PUBLIC_GOOGLE_PROJECT_ID,
  NEXT_PUBLIC_GOOGLE_STORAGE_BUCKET:
    process.env.NEXT_PUBLIC_GOOGLE_STORAGE_BUCKET,
  NEXT_PUBLIC_GOOGLE_MESSAGING_SENDER_ID:
    process.env.NEXT_PUBLIC_GOOGLE_MESSAGING_SENDER_ID,
  NEXT_PUBLIC_GOOGLE_APP_ID: process.env.NEXT_PUBLIC_GOOGLE_APP_ID,
  NEXT_PUBLIC_GOOGLE_MEASUREMENT_ID:
    process.env.NEXT_PUBLIC_GOOGLE_MEASUREMENT_ID,

  NEXT_PUBLIC_GOOGLE_AUTH_URI: process.env.NEXT_PUBLIC_GOOGLE_AUTH_URI,
  NEXT_PUBLIC_GOOGLE_TOKEN_URI: process.env.NEXT_PUBLIC_GOOGLE_TOKEN_URI,
  NEXT_PUBLIC_GOOGLE_REDIRECT_URIS:
    process.env.NEXT_PUBLIC_GOOGLE_REDIRECT_URIS,
  NEXT_PUBLIC_GOOGLE_CLIENT_ID: process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID,

  NEXT_PUBLIC_GEO_API_KEY: process.env.NEXT_PUBLIC_GEO_API_KEY,
  NEXT_PUBLIC_MAP_TOKEN: process.env.NEXT_PUBLIC_MAP_TOKEN,
  NEXT_PUBLIC_BE_URL: process.env.NEXT_PUBLIC_BE_URL,
})
if (!configProject.success) {
  console.error(configProject.error.issues)
  throw new Error("Các giá trị khai báo trong file .env không hợp lệ")
}

const envConfig = configProject.data
export default envConfig
